...
metadata:
  ...
  prompts:
    - "Using the Format MCP and the cube-roadmap-check skill, check this roadmap: [paste roadmap items]"
...
**The number to lead with, everywhere on the board, is recurrence: how many distinct customers each need recurs across.** That count is the one thing a team can't reliably assemble by hand — hand counts only see the deals someone deep-read, so a need logged at two customers can be visible at fourteen in the conversations. It opens the stage-setting paragraph, leads each item's row, and ranks the unbuilt-demand list, never trailing as a supporting detail. Recurrence is still a fact, not a grade: presenting it first is presentation of what's countable, and the conclusions stay the reader's. One trap: a group's `customerCount` is lifetime — it ignores the run's window — while the board's per-item company counts are windowed. Lead with the windowed count, label any lifetime figure as such ("N customers all-time"), and never present the two as the same number.
...
3. **Adjudicate strictly.** Semantic similarity is not demand. Bucket every candidate: **direct ask** (explicitly requests the capability), **implied need** (describes pain the item would resolve), **adjacent** (same area, different need — discard), **counter-evidence** (wants the opposite, or describes the item's approach as a problem — keep, shown separately). Judge the words themselves. Each row carries `isAiRejected` — Format's quality filter's verdict on the extraction. Treat it as a prompt for closer reading, never as a gate: a flagged row that adjudication accepts on its words still counts, and no row is discarded for the flag alone — the filter judges the extracted span, and it can flag the decisive quote.
...
**A recurring board belongs in a series.** If this check is going to repeat — the same roadmap re-checked each cycle — publish each run into one **series** rather than as another unrelated report: readers get one place to follow, and each board arrives numbered. The first time a repeat is expected, write the report, then `create_series({ orgId, name, reportId })` — the series is born from that report, which becomes edition 1; name the standing subject ("Roadmap evidence check"), never the run ("Roadmap check — March"). On a later run, find the series instead of founding a second one: `list_reports` rows carry `seriesId`, `seriesName` and `editionNumber`, so take the previous board's `seriesId` and pass it to `create_report`; `add_report_to_series` moves in a report already written. Appending is the series owner's right — if the series belongs to someone else, say so rather than quietly founding a duplicate. And set one expectation: the series name and numbering appear to readers once a second edition is published; until then the first edition reads as an ordinary one-off, and nothing is wrong.
...
For a deep dive on a single item — full research depth, derived requirements, an evidence page for one ticket — use **`cube-ticket-research`** if it's available: suggest it by name when the user zooms into one item ("want me to run cube-ticket-research on this one?"). If it isn't installed, don't dead-end: offer to do the deeper pass inline in this conversation, and mention that the skill exists in the Format skills gallery.

## How to prompt this skill

```
Using the Format MCP and the cube-roadmap-check skill, check this roadmap:
