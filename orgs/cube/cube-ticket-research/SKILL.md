...
metadata:
  ...
  prompts:
    - "Using the Format MCP and the cube-ticket-research skill, research this ticket: [paste ticket]"
...
This skill goes deep on **one** ticket. Validating a whole roadmap or a list of items is a different job — that's what `cube-roadmap-check` is for.
...
- **Carry the linked company's CRM attributes with its evidence.** An insight's `company` is identity only — after adjudication, and only when `describe_org`'s `attributes[]` lists deal-relevant fields, resolve each distinct linked company once with `get_company({ companyId })`, which returns the org's mapped `attributes`. Put the deal-relevant ones (deal value, opportunity stage and close date, account ARR, renewal date, account type — whatever this workspace maps) in the evidence map's deal-context column and the per-need company lines: "$87K open opp at Account Review" next to the company that said it is the difference between a quote and a priority, and it lets the reader's own register consume the page without a manual CRM join. Where the workspace maps no such attributes, skip the resolution calls and the deal-context column entirely; where a company is `inferred` rather than `linked`, show its evidence unannotated rather than guessing.
...
**A refreshed page belongs in a series.** If this research is going to be re-run — the same question re-checked after a release, a standing follow-up — publish each refresh into one **series** rather than as another unrelated report: readers get one place to follow, and each edition arrives numbered. The first time a repeat is expected, write the report, then `create_series({ orgId, name, reportId })` — the series is born from that report, which becomes edition 1; name the standing question, never the run. On a later run, find the series instead of founding a second one: `list_reports` rows carry `seriesId`, `seriesName` and `editionNumber`, so take the previous edition's `seriesId` and pass it to `create_report`; `add_report_to_series` moves in a report already written. Appending is the series owner's right — if the series belongs to someone else, say so rather than quietly founding a duplicate. And set one expectation: the series name and numbering appear to readers once a second edition is published; until then the first edition reads as an ordinary one-off, and nothing is wrong.
...
```
Using the Format MCP and the cube-ticket-research skill, research this ticket:
