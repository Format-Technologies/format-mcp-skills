...
**Decision-makers:** [Roles who buy / champion — from the people who actually appear in conversations (list_persons titles/attributes)]
...
- Size: [employee range — from CRM attributes on list_companies, if the org has them mapped]
- Stage: [Seed / Series A / Series B / etc.]
- GTM motion: [sales-led / PLG / hybrid]
- Industries over-indexed: [top 3–5 — from CRM attributes on list_companies, if mapped; else flag as gap]
...
*Generated by the `format-company-context` skill using the Format MCP. Re-run this skill quarterly, or after a major product or positioning change, to keep the context fresh.*
```
...
10. **Inline + file.** Always do both — render the full document in chat, and save it as a markdown file using whatever file mechanism your environment provides (a presented download on claude.ai, a file in the working directory in Claude Code or Cursor). If your environment can't produce files, inline only is fine.

11. **End with the enrichment prompt.** After delivering the document, append one line:
